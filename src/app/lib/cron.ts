...
export const deleteUnverifiedfieldDoctors = async () => {
    cron.schedule('*/10 * * * *', async() => {
        try {
            ...
            if(deleteDoctors.count > 0){
                console.log(`Cron: Deleted ${deleteDoctors.count} unverified email doctor applications older than 1 hour`);
            }
        } catch (error) {
    ...
    console.log("Unverified Doctor Delete cron scheduled every 10 minutes");
}